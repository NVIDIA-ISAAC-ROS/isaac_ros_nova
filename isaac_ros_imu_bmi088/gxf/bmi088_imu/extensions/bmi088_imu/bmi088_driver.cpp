...
constexpr double kStartupTimeS = 0.035;  // Time after startup to wait until data is valid (35ms)
...
gxf_result_t Bmi088Driver::registerInterface(gxf::Registrar* registrar) {
  ...
  result &= registrar->parameter(
      ...
      "Frequency to set the accelerometer to.", 200);
  result &= registrar->parameter(
      gyro_frequency_, "gyro_frequency", "Gyroscope Update Frequency (Hz)",
      "Frequency to set the gyroscope to.", 200);
  result &= registrar->parameter(
      bmi_id_, "bmi_id", "BMI088 ID",
      "Id for BMI088 IMU on Carter_v2.3 you can expect one IMU with ID 69, on Carter_v2.4 there "
      "are two IMUs, one with ID 68 and the other with 69",
      69);
...
gxf_result_t Bmi088Driver::start() {
  GXF_LOG_INFO("Starting bmi088 imu driver");
...
gxf::Expected<int> Bmi088Driver::getBmiId(const std::string dev_path) {
  ...
  GXF_LOG_INFO("Found device at %s", dev_path.c_str());
  return bmi_id;
}
...
void Bmi088Driver::asyncDeviceThread(DeviceTypes device_type) {
  ...
  while (true) {
    ...
    if (main_thread_state_ == ThreadStates::kStopRequested) {
      GXF_LOG_INFO("Stop requested, shutting down");
      break;
    }
...
gxf::Expected<void> Bmi088Driver::initializeDevice(int device_index, float& scale, int& buf_fd) {
  GXF_LOG_INFO("Initializing iio device %i", device_index);
  ...
  GXF_RETURN_IF_ERROR(
      enableDisableAll(sysfs_path + "/scan_elements", 1), Severity::ERROR,
      "Failed to initialize device");

  scale = GXF_UNWRAP_OR_RETURN(
      readScaleFile(sysfs_path), Severity::ERROR, "Failed to read scale file!");

  GXF_RETURN_IF_ERROR(
      writeAndVerifyInt(sysfs_buf_path + "length", kIioBufSize), Severity::ERROR,
      "Failed to set length");

  GXF_RETURN_IF_ERROR(
      writeAndVerifyInt(sysfs_buf_path + "enable", 1), Severity::ERROR,
...
void Bmi088Driver::cleanupDevice(int device_index, int buf_fd) {
  ...
  GXF_LOG_INFO("Cleaning up device %i", device_index);
...
gxf::Expected<void> Bmi088Driver::publishAccelData() {
  AccelerometerMessageParts message = GXF_UNWRAP_OR_RETURN(CreateAccelerometerMessage(context()));
  {
    ...
    auto sw_timestamp_entity = GXF_UNWRAP_OR_RETURN(message.message.add<int64_t>("sw_timestamp"));
...
gxf::Expected<void> Bmi088Driver::publishGyroData() {
  GyroscopeMessageParts message = GXF_UNWRAP_OR_RETURN(CreateGyroscopeMessage(context()));
  {
...
gxf::Expected<void> Bmi088Driver::enableDisableAll(const std::string& directory_path, bool enable) {
  for (const auto& entry : std::filesystem::directory_iterator(directory_path)) {
    if (entry.is_regular_file() && endsWith(entry.path().stem().string(), "_en")) {
      ...
      GXF_RETURN_IF_ERROR(writeAndVerifyInt(filename, enable));
    }
  }
  return gxf::Success;
}
