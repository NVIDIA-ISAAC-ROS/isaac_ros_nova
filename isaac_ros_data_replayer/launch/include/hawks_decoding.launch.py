...
def create_decoder(stereo_camera_name: str, identifier: str,
    ...
    condition = all_types.IfCondition(
        lu.has_substring(args.enabled_stereo_cameras, stereo_camera_name))
    action = lu.load_composable_nodes(args.container_name, [composable_node], condition=condition)
    return action


def create_hawk_decoding(stereo_camera_name: str,
                         args: lu.ArgumentContainer) -> List[all_types.Action]:
    actions = []

    actions.append(create_decoder(stereo_camera_name, 'left', args))
    actions.append(create_decoder(stereo_camera_name, 'right', args))
...
def generate_launch_description() -> all_types.LaunchDescription:
    ...
    actions.extend(create_hawk_decoding('front_stereo_camera', args))
    actions.extend(create_hawk_decoding('back_stereo_camera', args))
    actions.extend(create_hawk_decoding('left_stereo_camera', args))
    actions.extend(create_hawk_decoding('right_stereo_camera', args))
